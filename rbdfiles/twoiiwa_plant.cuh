...
namespace gato_plant{
	...
	void forwardDynamics(T *s_qdd, T *s_q, T *s_qd, T *s_u, T *s_XITemp, void *d_dynMem_const, cooperative_groups::thread_block block){
    	...
    	grid::load_update_XImats_helpers<T>(s_XImats, s_q, s_topology_helpers, (grid::robotModel<float> *) d_dynMem_const, s_temp);
    	__syncthreads();

    	grid::forward_dynamics_inner<T>(s_qdd, s_q, s_qd, s_u, s_XImats, s_topology_helpers, s_temp, gato_plant::GRAVITY<T>());
	...
	template <typename T>
	__device__
	void forwardDynamicsGradient( T *s_dqdd, T *s_q, T *s_qd, T *s_u, T *s_temp_in, void *d_dynMem_const, cooperative_groups::thread_block block){
		...
		grid::inverse_dynamics_inner<T>(s_temp, s_vaf, s_q, s_qd, s_XImats, s_topology_helpers, &s_temp[14], GRAVITY<T>());
		grid::forward_dynamics_finish<T>(s_qdd, s_u, s_temp, s_Minv);
		grid::inverse_dynamics_inner_vaf<T>(s_vaf, s_q, s_qd, s_qdd, s_XImats, s_topology_helpers, s_temp, GRAVITY<T>());
    ...
    void forwardDynamicsAndGradient(T *s_dqdd, T *s_qdd, T *s_q, T *s_qd, T *s_u,  T *s_temp_in, void *d_dynMem_const, cooperative_groups::thread_block block){
		...
		__syncthreads();
		// grid::forward_dynamics_gradient_device<T>(s_dqdd, s_qdd, s_q, s_qd, s_u, (grid::robotModel<T> *)d_dynMem_const,GRAVITY<T>());
    }
