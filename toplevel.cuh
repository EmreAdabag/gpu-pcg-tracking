...
void track(uint32_t state_size, uint32_t control_size, uint32_t knot_points, const uint32_t traj_steps, float traj_timestep, T *d_traj, T *d_traj_lambdas, T *d_xs){
    ...
    while(1){
        ...
        cur_timestep = interpolate_knotpoints<T>(state_size, control_size, knot_points, traj_steps, traj_timestep, time_since_start, d_traj_lambdas, d_lambda, d_xu_goal, d_traj);
        gpuErrchk(cudaPeekAtLastError());
